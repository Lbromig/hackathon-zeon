...
- **The wired floor path needs a taught bench to move (demo-day, requires hardware; teaching UNSTALLED this cycle).**
  `_execute` replays named taught poses; the real bench library (`data/teach_poses.json`, gitignored) jumped from **2 to 8
  poses across both arms** this cycle — right (6): `cap_grasp_approach`, `cap_grasp`, `tube_grasp_approach`, `tube_grasp`,
  `transport_safe`, `present_approach`; **left (2, newly taught): `tube_hold_approach`, `tube_hold`** (saved 05:14–05:24Z).
  The four-review right-arm-only stall is broken and the **second arm is now being taught on the real rig.** Still untaught:
  `cap_lift`, `cap_dropoff`, `cap_dropoff_retreat`, the `present_ot`/aspirate poses, and more left-arm poses, so `preflight`
  still correctly refuses the full choreography — finish teaching them on the two-arm + OT rig (cannot be pre-staged from a
  laptop) and the floor path moves (Q-POSES-1).
...
- **Allocation drift — easing, and the threat has narrowed to a single item (Q-OT-1).** For four cycles this was the #1
  threat: laptop work over room work. **This cycle the team went to the room.** Pose-teaching unstalled (2 → 8 poses, the
  **left arm taught for the first time**, saved 05:14–05:24Z on the real rig — Q-POSES-1), the kinematics loop was
  **committed** (Q-KIN-1), and the reparent line was written (Q-TWIN-COUPLING) — two of the three deciding items moved,
  one of them room-only. The allocation pattern is breaking. What remains is now **one room-only item: the Opentrons serial
  transport (Q-OT-1)** — `feat/ot-one-serial-driver` still unmerged, the pipette still can't draw, and it is the narrative
  climax. That is the whole ballgame for the next block: **merge the OT serial driver and finish teaching the remaining
  poses at the bench**, then commit the reparent WIP and measure the 210/211 board spacing (one caliper reading) so the
  committed world frame is metrically real (Q-OT-1, Q-POSES-1, Q-TWIN-COUPLING, Q-CALIB-1).
