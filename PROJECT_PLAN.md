...
- **A large, well-tested batch of new code is uncommitted (WIP-loss risk, a repeat of the resolved Q-COMMIT-1).** No team
  commit landed this cycle, yet the working tree holds the ratchet-unscrew + tests (Q-CAPOPS-1), path-teaching, the reparent
  WIP + its new test, and teach-API/frontend changes — all `py_compile`-clean. A clean checkout at hour 24 would lack all of
  it. Commit the batch onto `agent-loop-p0` (Q-COMMIT-2).
