...
- **The verify loop's parent-based half is still hollow on the live bench — the motion half is now wired, the reparent half is not (verification honesty).**
  This cycle the **arm-FK → twin** motion coupling landed on disk (uncommitted): `core/kinematics.py` +
  `services/kinematics.py` write each arm's live TCP into the twin at ~12 Hz, so the twin's arms finally move with the
  real hardware (Q-KIN-1). But `reparent()` is **still** exercised **only in tests** — **zero** `reparent` calls in
  production `backend/app/`/`core/`, and `_run_act`'s `grip`/`release` never attach the tube to the tool. So
  `grasp_secure` (tube parented to a `TOOL`) and `cap_removed`'s reparent clause still can never turn true from a real
  grasp; the retry loop degrades to perception-geometry-only for those steps — which the now-moving twin at least makes
  honest for `tube_aligned`/`cap_removed`-separation. For Track C — whose *other* half is physical verification — this
  is as on-theme a threat as the OT no-op. Write the one-line `wm.reparent("tube_1","<tool>")` into `_execute`
  (the tests spell it out; the RLock and FK loop are both ready) **and commit the kinematics loop**, or formally scope
  the demo's verify moment onto the geometry-only predicate (Q-TWIN-COUPLING, Q-KIN-1; team `docs/INTEGRATION_PLAN.md`).
...
- **Allocation drift — still the #1 threat (fourth straight cycle), now with a sharper edge.** This cycle finally put
  new work **on** the critical path — the arm-FK → twin kinematics loop (Q-KIN-1) — which is genuine progress and the
  right instinct. But it is the **laptop-doable half** of Q-TWIN-COUPLING (the one-line `reparent` that greens the
  verification climax is *still* unwritten, now with the RLock substrate **and** the FK loop both sitting ready beside
  it), it is **uncommitted** (real critical-path code not in git — the Q-COMMIT-1 failure recurring), and it left the
  two **room-only** deciding items untouched a fourth time: the OT serial merge (Q-OT-1) and pose-teaching
  (Q-POSES-1, still 2/12). The pattern is now precise and worth naming bluntly: the team keeps doing the version of the
  next deciding item that can be done at a laptop, and keeps deferring the version that can only be done in the room on
  the hardware. Perimeter and laptop work have no deadline; the OT rig and pose-teaching only happen at the bench and
  will eat the final hour if left late. Redirect the next block to the two-arm+OT rig and the OT merge; write the one
  `reparent` line and **commit** the kinematics loop while at it; and measure the 210/211 board spacing (one caliper
  reading) so the committed world frame is metrically real (Q-ALLOC-1, Q-KIN-1, Q-OT-1, Q-POSES-1, Q-CALIB-1).
