...
- **Allocation drift — ESCALATED, now the #1 threat (third straight cycle).** The last two cycles' effort
  (still-cam hedge, worldmodel concurrency guard, then this cycle's committed world-frame calibration + world-map viz
  + remote camera driver) is honest, useful work — and the team *did* follow through on one prior recommendation by
  committing the calibration WIP, which proves they close items when they choose to. But the three deciding items
  (OT serial merge, finish teaching 12 poses, wire one reparent line *or* adopt a geometry-only verify moment) have
  now **stood still for three reviews running**, both narrative climaxes remain mimed, and the bench is still at 2/12.
  The deciding risk is no longer capability — it is **time allocation**. Perimeter work has no deadline and expands to
  fill any hour it's given; the OT rig and pose-teaching only happen in the room and will eat the final hour if left
  late. Redirect the next block to the two-arm+OT rig and the OT merge — and measure the 210/211 board spacing (one
  caliper reading) so the now-committed world frame is metrically real (Q-ALLOC-1, Q-CALIB-1).
